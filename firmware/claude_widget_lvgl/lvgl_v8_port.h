...
// Confirmed root cause 2026-09-02: with these buffers (~62KB at the default
// buffer height) sitting in internal SRAM, mbedTLS's own TLS handshake buffer
// couldn't find a large-enough contiguous free block, even with tens of KB of
// *total* free heap available — "SSL - Memory allocation failed", 100%
// reproducible, unrelated to timing/retries/cleanup (all tried and ruled out
// first). PSRAM is a few % slower for LVGL's flush, which is a non-issue for
// this appliance-style UI; freeing that internal SRAM is what actually lets
// HTTPS calls to Square succeed at all.
...
#else
// 3 = LCD double-buffer + LVGL direct mode. Flushes land in the back buffer and swap
// on VSYNC, instead of being written into the frame the panel is scanning out -
// which showed as flashing lines and the image jumping. Needs the bus configured
// with 2 frame buffers (see configRGB_FrameBufferNumber in boardInit()).
