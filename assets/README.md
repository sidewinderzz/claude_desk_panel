...
# `font_clock_112` — the clock face

`ui/font_clock_112.c` is Montserrat Medium at 112 px, 4-bit anti-aliased, with
only `-./0123456789:` in it (14 glyphs, ~30 KB of flash). It is generated by
`make_clock_font.ps1` from the TTF that ships inside the LVGL library
(`libraries/lvgl/scripts/built_in_font/Montserrat-Medium.ttf`), using the same
converter LVGL uses for its built-in faces, `lv_font_conv`, run through `npx`.
Nothing is downloaded except the converter.

```powershell
.\make_clock_font.ps1        # rewrites ui/font_clock_112.c
```

Two things to keep straight if you change it:

- `--no-compress` is required. `lv_conf.h` has `LV_USE_FONT_COMPRESSED 0`, and a
  compressed font on that build renders nothing, with no error.
- Any character outside the range simply does not draw. The clock label only ever
  holds digits, `:` and the `--:--` placeholder, so that is the whole range.
